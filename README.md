...
- OCR 模式可切换（快速 / 均衡 / 高精度）
- OCR 准确率增强（多路预处理 + 暗色页面反相 + 质量评分筛选）
- OCR 结果可视化诊断（模式、最佳分支、评分、各尝试结果）
- 文本抽取增强（DOM 长度/质量双阈值 + DOM/OCR 融合补全）
- PDF 清晰度增强（更高导出宽度 + 更高质量回退 + 无损阈值上调）
...
2. 点击扩展图标，设置：
   ...
   - OCR 兜底（可选）：启用开关、选择模式、语言、填写 OCR.Space API Key
3. 点击“开始”
4. 需要中止时点击“停止”
5. 在状态区可查看 OCR 诊断明细（每次 OCR 的分支尝试结果）
